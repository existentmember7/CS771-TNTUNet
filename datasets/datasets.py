...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        print(f"img name: {self.filenames[idx]}")
        ...
        # color = cv2.resize(cv2.imread(os.path.join(self.data_path,'color',"1_000120.png")), (self.opt.image_width, self.opt.image_height), interpolation = cv2.INTER_AREA)
        ...
        # label = cv2.resize(cv2.imread(os.path.join(self.data_path,'label',"1_000120.png"), cv2.IMREAD_UNCHANGED), (self.opt.image_width, self.opt.image_height), interpolation = cv2.INTER_AREA)
